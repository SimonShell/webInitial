<template>
    <div
        ...
        <div ref="reference" :class="[prefixCls + '-rel']" @click="handleFocus">
            <slot>
                <YYInput
                    ...
                    <div :class="[prefixCls + '-suffix']" slot="suffix">
                        <Icon 
                            v-if="clearable && visualValue && isMouseEnter" 
                            @click.native.stop="handleClear"
                            type="close-circle-fill"/>
                        <Icon v-else type="calendar"/>
                    </div>
                </YYInput>
            </slot>
        </div>
...
<script>
    ...
    const pulseElement = (el) => {
        const pulseClass = 'yy-date-picker-btn-pulse'
    ...
    const prefixCls = 'yy-date-picker'
    const pickerPrefixCls = 'yy-picker'
    export default {
        name: 'YYPicker',
        ...
        data() {
            ...
            return {
                ...
                internalFocus: false,
                defaultPlaceholder: this.type === 'time' || this.type === 'timerange' ? this.t('datepicker.selectTime') : this.t('datepicker.selectTime')
            }
        },
        computed: {
            wrapperClasses() {
        ...
        methods: {
                ...
                if (keyCode === 9) {
                    if (this.visible) {
                        ...
                        if (this.isConfirm) {
                            ...
                            const tabbable = this.$refs.dropdown.$el.querySelectorAll(selector)
            ...
            navigateDatePanel(direction, shift) {
                ...
                if (shift) {
                    ...
                    // pulse button
                    const button = this.$refs.dropdown.$el.querySelector(`.yy-date-picker-${position}-btn-arrow${double}`)
            ...
            handleClear() {
                ...
                this.$emit('on-clear')
                this.dispatch('YYFormItem', 'on-form-change', '')
                this.emitChange(this.type)
                this.reset()
            ...
            emitChange(type) {
                this.$nextTick(() => {
                    ...
                    this.dispatch('YYFormItem', 'on-form-change', this.publicStringValue)
                })
            },
            parseDate(val) {
                const isRange = this.type.includes('range')
